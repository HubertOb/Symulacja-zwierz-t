...
public class Map1 implements IWorldMap,IPositionChangeObserver{
    private final int width;
    private final int height;
    private final int plantEnergy;
    private final int startEnergy;
    private final Chart lineChart;
    private int magicEvolution=0;
    private final MakeCSVFile file;
    ...
    public Map1(int w, int h,float ratio,int energy,int s,Chart l,MakeCSVFile f){
        width=w;
        height=h;
        plantEnergy=energy;
        startEnergy=s;
    ...
    public void moveAnimals(int moveEnergy) {
        ...
        for (Animal animal:animals.values()){
            if (animal.getEnergy()>moveEnergy){
                animal.map1Move(moveEnergy,width,height);
